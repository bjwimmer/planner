...
// --- Robust "Add thread" fallback (works even if page init changes) ---
function __plannerAddThreadFallback(){
  const titleEl = document.querySelector('[data-thread-title]') || document.querySelector('#threadTitle') || document.querySelector('input[name="threadTitle"]');
  const domainEl = document.querySelector('[data-thread-domain]') || document.querySelector('#threadDomain') || document.querySelector('input[name="threadDomain"]');
  const nextEl  = document.querySelector('[data-thread-next]')  || document.querySelector('#threadNext')  || document.querySelector('input[name="threadNext"]');
  const name = (titleEl?.value || '').trim();
  const domain = (domainEl?.value || '').trim();
  const nextAction = (nextEl?.value || '').trim();
  if(!name){
    alert("Add a thread title first.");
    titleEl?.focus?.();
    return;
  }
  // Uses existing data model: data.threads array
  ...
  const t = {
    ...
    nextAction: nextAction || "First step: [describe 5-20 minute action]",
    lastTouched: now
  };
  try{
    const data = loadData();
    data.threads = Array.isArray(data.threads) ? data.threads : [];
    data.threads.unshift(t);
    saveData(data);
    // Re-render if page has renderer; otherwise reload
    if(typeof initThreadRegistry === "function"){
      try{ initThreadRegistry(); }catch(e){ location.reload(); }
    }else{
      location.reload();
    }
  }catch(e){
    console.error("Add thread failed", e);
    alert("Add thread failed (see console).");
  }
}

document.addEventListener("click", (ev)=>{
  const btn = ev.target.closest('[data-add-thread], #addThreadBtn');
  if(btn){
    ev.preventDefault();
    __plannerAddThreadFallback();
  }
});
document.addEventListener("submit", (ev)=>{
  const form = ev.target.closest('[data-create-thread-form], #createThreadForm');
  if(form){
    ev.preventDefault();
    __plannerAddThreadFallback();
  }
