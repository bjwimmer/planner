...
// --- GLOBAL_FALLBACK_NEW_THREAD_FORM ---
// If page init fails for any reason, we still want "Add thread" to work.
document.addEventListener("submit", (e)=>{
  try{
    const form = e.target;
    if(!form || form.id !== "newThreadForm") return;
    e.preventDefault();

    const st = loadState();
    const titleEl = document.querySelector("#tTitle");
    const domainEl = document.querySelector("#tDomain");
    const nextEl  = document.querySelector("#tNext");
    ...
    saveState(st);
    // quick, reliable refresh so the new card appears even if render code is busted
    location.reload();
  }catch(err){
    console.error(err);
    alert("Add thread failed (unexpected).");
  }
}, true);
